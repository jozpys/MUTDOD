...
namespace BasicIndexes
{
    public class StringIndex : IIndex<Type>
    {
        ...
        public IndexData RemoveObject(IndexData indexData, Oid obj, string[] attributes)
        {
            ...
            foreach (String attribute in attributes)
            {
                if (objFields.Single(p => p.Name == attribute).GetType() == typeof (string))
                {
                    string attributeValue = objFields.Where(p => p.Name == attribute).Single().GetValue(obj).ToString();
    ...
    {
        public TBinarySTree<Type, TBinarySTree<string, TBinarySTree<string, List<Guid>>>> BT;
        ...
        private TBinarySTree<string, TBinarySTree<string, List<Guid>>> GetAttribiutesBT(Type t)
        {
            var tree = BT.find(t);
            if (tree == null)
        ...
        private TBinarySTree<string, List<Guid>> GetObjectsBT(Type t, string attribiute)
        {
        ...
        private List<Guid> GetObjectsBT(Type t, string attribiute, string value)
        {
        ...
        public void AddToBTvalue(Oid obj, string attribiute, string value)
        {
        ...
        public void RemoveFromBTvalue(Oid obj, string attribiute, string value)
        {
            var values = GetObjectsBT(obj.GetType(), attribiute, value);
            if (!values.Contains(obj.Id))
                values.Add(obj.Id);
